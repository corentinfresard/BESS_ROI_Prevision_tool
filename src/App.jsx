...
function batteryPriceFromCurve(kwh, anchors, customBattery) {
  if (
    customBattery?.kwh > 0 &&
    customBattery?.price > 0 &&
    Math.abs(kwh - customBattery.kwh) < 1
  )
...
function detectProfile(data) {
  ...
  if (pvKWh < consoKWh * 0.05)
    return {
      ...
      pvKWh,
      consoKWh,
      reason: `Production PV négligeable (${(pvRatio * 100).toFixed(
        0
      )}% de la conso)`,
    };
  ...
  const psScore =
    Math.min(1, (peakFactor - 2) / 8) * (1 - Math.min(1, pvRatio * 1.5));

  if (acScore > psScore * 1.3)
    return {
      mode: 'autoconso',
      pvRatio,
      surplusRatio,
      ...
      pvKWh,
      consoKWh,
      reason: `PV important (${(pvRatio * 100).toFixed(
        0
      )}% de la conso) avec ${(surplusRatio * 100).toFixed(
        0
      )}% de surplus exporté`,
    };
  if (psScore > acScore * 1.3)
    return {
      mode: 'peakshaving',
      pvRatio,
      surplusRatio,
...
function minBatteryPS(data, thresholdKW, baseMaxKW, hpHc) {
  ...
  for (let i = 0; i < 28; i++) {
    const mid = (lo + hi) / 2;
    runSim(data, thresholdKW, mid, 'peakshaving', hpHc).maxGridKW <=
    thresholdKW * 1.005
      ? (hi = mid)
      : (lo = mid);
  }
...
function computePSCurve(data, prixAchat, prixVente, prixPuissance, hpHc) {
  ...
  for (let pct = 0; pct <= 99; pct++) {
    ...
    const savPui =
      basePuiCost - sim.monthlyPeaks.reduce((s, p) => s + p * prixPuissance, 0);
    const savEn = isHpHc
...
function generateDemoData() {
  ...
  for (let i = 0; i < 365 * 96; i++) {
    ...
    const cKW = Math.max(
      0,
      base + mP + eP + spike + (Math.random() - 0.5) * 0.5
    );
...
function BeforeAfter({
  ...
  const basePuiCost = base.monthlyPeaks.reduce(
    (s, p) => s + p * prixPuissance,
    0
  );
  const simPuiCost = sim.monthlyPeaks.reduce(
    (s, p) => s + p * prixPuissance,
    0
  );
    ...
    <div className="compare-grid">
      <div className="compare-card">
        ...
        <div className="compare-row">
          <span className="label">Puissance crête réseau</span>
          <span className="value">{base.maxGridKW.toFixed(1)} kW</span>
        </div>
        <div className="compare-row">
          <span className="label">Achat réseau annuel</span>
          <span className="value">{fmt(base.gridKWh)} kWh</span>
        </div>
        <div className="compare-row">
          <span className="label">Coût énergie annuel</span>
          <span className="value" style={{ color: 'var(--red)' }}>
            {fmt(baseCostEn)} CHF
          </span>
        </div>
        <div className="compare-row">
          <span className="label">Coût puissance annuel</span>
          <span className="value" style={{ color: 'var(--red)' }}>
            {fmt(basePuiCost)} CHF
          </span>
        </div>
      ...
      <div className="compare-card after">
        <div className="compare-title after">
          Situation après batterie optimale
        </div>
        <div className="compare-row">
          <span className="label">Puissance crête réduite</span>
          <span className="value" style={{ color: 'var(--green)' }}>
            {sim.maxGridKW.toFixed(1)} kW
          </span>
        </div>
        <div className="compare-row">
          <span className="label">Réduction pic</span>
          <span className="value" style={{ color: 'var(--green)' }}>
            {Math.round((1 - sim.maxGridKW / base.maxGridKW) * 100)}%
          </span>
        </div>
        <div className="compare-row">
          <span className="label">Taille batterie</span>
          <span className="value" style={{ color: 'var(--blue)' }}>
            {capKWh.toFixed(0)} kWh ({(capKWh * C_RATE).toFixed(0)} kW)
          </span>
        </div>
        <div className="compare-row">
          <span className="label">Économies annuelles</span>
          <span className="value" style={{ color: 'var(--green)' }}>
            {fmt(savEn + savPui)} CHF
          </span>
        </div>
      </div>
    </div>
...
export default function App() {
  ...
  const runComputation = useCallback(
    async (rows, mode, pa, pv, pp, hh, returnTab = 'overview') => {
      setPhase('computing');
      ...
      if (mode === 'peakshaving') {
        ...
        const safeRate = Math.min(
          optRate,
          result.points[result.points.length - 1]?.rate ?? optRate
        );
        setPsCurve(result);
  ...
  const customBattValid = useMemo(() => {
    const k = parseFloat(customBattery.kwh),
      p = parseFloat(customBattery.price);
    return k > 0 && p > 0 ? { kwh: k, price: p } : null;
  }, [customBattery]);

  const getPrice = useCallback(
    (kwh) => batteryPriceFromCurve(kwh, priceAnchors, customBattValid),
  ...
  const acScenario = useMemo(() => {
    ...
    const pt =
      acCurve.points.find((p) => Math.abs(p.capKWh - selectedAcCap) < 1) ??
      acOptimal ??
      acCurve.points[0];
    if (!pt) return null;
    ...
    <div style={{ maxWidth: 700 }}>
      <div className="card">
        ...
        <div style={{ marginBottom: 24 }}>
          <div className="toggle-row">
            <label className="toggle">
              <input
                ...
                onChange={(e) => {
                  ...
                  setHpHcEnabled(v);
                  handleTariffChange(prixAchat, prixVente, prixPuissance, {
                    enabled: v,
                    hp: prixHP,
                    hc: prixHC,
                  });
            ...
            <span className="toggle-label">
              Tarification heure pleine / heure creuse (HP/HC)
            </span>
          </div>
          {hpHcEnabled && (
            <div
            ...
            >
              ...
              >
                Avec HP/HC : la batterie charge aussi en heures creuses pour
                arbitrage tarifaire. Les économies d'énergie utilisent le coût
                réel HP/HC.
              </div>
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          {[
            {
              label: "Tarif d'achat électricité",
              val: prixAchat,
              set: (v) => {
                setPrixAchat(v);
                handleTariffChange(v, prixVente, prixPuissance, hpHc);
              },
              unit: 'kWh',
              step: 0.01,
              hide: hpHcEnabled,
            ...
            {
              label: 'Tarif de vente (injection)',
              val: prixVente,
              set: (v) => {
                setPrixVente(v);
                handleTariffChange(prixAchat, v, prixPuissance, hpHc);
              },
              unit: 'kWh',
              step: 0.01,
            },
            {
              ...
              set: (v) => {
                setPrixPuissance(v);
                handleTariffChange(prixAchat, prixVente, v, hpHc);
              },
              unit: 'kW/mois',
              step: 0.5,
            },
        ...
        >
          ...
          >
            {[
              ...
              <div key={p.label}>
                <div className="cfg-label" style={{ fontSize: 9 }}>
                  {p.label}
                </div>
                <div className="inp-row">
                  <DeferredInput
  ...
  const RoiPanel = () => {
    ...
    // ROI KPI
    const currentSavings = isPS
      ? psScenario?.annualSavings
      : acScenario?.annualSavings;
    ...
    const roiPoints = isPS
      ? psRoiCurve
      : acCurve.points.map((p) => {
          const prix = getPrice(p.capKWh);
          const roi = computeRoiWithInflation(
            prix,
            p.annualSavings,
    ...
    const optROIPoint = validRoi.length
      ...
      <>
        ...
        <div className="res-grid" style={{ marginBottom: 24 }}>
          <div className="rcard hl-green">
            ...
            <div className="rs">
              {isPS
                ? `Puissance : ${fmt(
                    psScenario?.savPui ?? psScenario?.savingsPuissance
                  )} CHF · Énergie : ${fmt(
                    psScenario?.savEn ?? psScenario?.savingsEnergy
                  )} CHF`
                : `Énergie : ${fmt(acScenario?.savEn)} CHF · Puissance : ${fmt(
                    acScenario?.savPui
                  )} CHF`}
            </div>
          </div>
          <div
            className={`rcard ${
              currentRoi ? (currentRoi < 10 ? 'hl-green' : 'hl-amber') : ''
            }`}
          >
            <div className="rl">ROI (inflation {inflation}%)</div>
            <div className="rv">
              {currentRoi != null
                ? `${currentRoi.toFixed(1)} ans`
                : currentPrix > 0
                ? '> 40 ans'
                : '—'}
            </div>
            <div className="rs">
              Investissement : {fmt(currentPrix)} CHF · {currentKwh?.toFixed(0)}{' '}
              kWh
            </div>
            {currentRoi && (
              <div className="roi-track">
                <div
                  className="roi-fill"
                  style={{
                    width: `${Math.min(100, (currentRoi / 15) * 100)}%`,
                    background:
                      currentRoi < 8 ? 'var(--green)' : 'var(--amber)',
                  }}
                />
              </div>
            )}
          ...
          <div className="rcard hl-blue">
            <div className="rl">
              {isPS
                ? 'Taux optimal (ROI min.)'
                : 'Capacité optimale (ROI min.)'}
            </div>
            <div className="rv">
              {optROIPoint
                ? isPS
                  ? `${optROIPoint.rate}%`
                  : `${optROIPoint.kwh?.toFixed(0)} kWh`
                : '—'}
            </div>
            <div className="rs">
              {optROIPoint
                ? `ROI ${optROIPoint.roi?.toFixed(1)} ans · ${fmt(
                    optROIPoint.prix
                  )} CHF`
                : '—'}
            </div>
        ...
        <div className="two-col" style={{ marginBottom: 24 }}>
          <div className="sec">
            <div className="sec-title">
              <span className="dot" />
              Économies nettes par {isPS ? 'taux' : 'taille de batterie'}
            </div>
            <div className="card">
              <ResponsiveContainer width="100%" height={230}>
                ...
                >
                  ...
                  <XAxis
                    dataKey={isPS ? 'rate' : 'kwh'}
                    stroke="#c8d4e3"
                    tickLine={false}
                    tick={{ fontSize: 11, fill: '#7a90a8' }}
                    label={{
                      value: isPS ? 'Taux (%)' : 'Batterie (kWh)',
          ...
          <div className="sec">
            <div className="sec-title">
              ...
              ROI par {isPS ? 'taux' : 'taille de batterie'}
            </div>
            <div className="card">
              <ResponsiveContainer width="100%" height={230}>
                <LineChart
        ...
        <div className="sec">
          ...
          <div className="card">
            ...
            >
              <div>
                <div className="cfg-label" style={{ marginBottom: 10 }}>
                  Points de contrôle
                </div>
                <table
                  style={{
                ...
                >
                  ...
                  <tbody>
                    {priceAnchors.map((a, idx) => (
                      ...
                      >
                        <td
                          style={{ padding: '5px 10px', color: 'var(--text2)' }}
                        >
                          {a.kwh} kWh
                        </td>
                        <td style={{ padding: '5px 10px' }}>
                          <div className="inp-row" style={{ maxWidth: 110 }}>
                            <DeferredInput
                              ...
                              onChange={(v) => {
                                const n = [...priceAnchors];
                                n[idx] = { ...n[idx], chfPerKwh: v };
                                setPriceAnchors(n);
                              }}
                            />
                            <span
                ...
                {customBattValid && (
                  ...
                  >
                    ★ Offre active : {customBattValid.kwh} kWh à{' '}
                    {fmt(customBattValid.price)} CHF (
                    {fmt(customBattValid.price / customBattValid.kwh)} CHF/kWh)
                  </div>
                )}
              </div>
              <div>
                  ...
                  >
                    ...
                    <Tooltip
                      content={({ active, payload, label }) => {
                        ...
                        return (
                          ...
                          >
                            <div style={{ color: '#7a90a8', marginBottom: 4 }}>
                              {label} kWh
                            </div>
                            <strong style={{ color: '#1a6fbf' }}>
                            ...
                            >
                              {label > 0
                                ? Math.round(payload[0].value / label)
                                : 0}{' '}
                              CHF/kWh
                            </div>
                          </div>
                    ...
                    {customBattValid && (
                      <ReferenceLine
                        ...
                        strokeDasharray="4 3"
                        label={{
                          value: `★ ${fmt(customBattValid.price)} CHF`,
                          fill: '#d97706',
                          fontSize: 10,
                          position: 'top',
                        }}
              ...
              >
                <label style={{ display: 'block', cursor: 'pointer' }}>
                  ...
                  <div className="uz-hint">
                    Colonnes : <strong>timestamp</strong> ·{' '}
            ...
            <div className="profile-select">
              ...
              >
                <div
                  className={`profile-card ${
                    profile.mode === 'peakshaving' ? 'selected' : ''
                  }`}
                ...
                >
                  ...
                  <div className="profile-desc">
                    Réduire la puissance crête souscrite au réseau. Idéal si
                    votre client paie un tarif de puissance élevé ou a des pics
                    de demande marqués.
                    <br />
                    <br />
                    <strong>EMS prédictif :</strong> charge préventive nocturne,
                    anticipation des pics.
                  </div>
                  {profile.mode === 'peakshaving' && (
                    <div className="profile-badge suggested">
                      ★ Recommandé pour ce profil
                    </div>
                  )}
                  {profile.mode !== 'peakshaving' && (
                    <div className="profile-badge alt">Option alternative</div>
                  )}
                </div>
                <div
                  className={`profile-card ${
                    profile.mode === 'autoconso' ? 'selected' : ''
                  }`}
                ...
                >
                  ...
                  <div className="profile-desc">
                    Maximiser l'utilisation de la production PV sur site. Idéal
                    si votre client a une grande installation PV et veut réduire
                    ses achats réseau.
                    <br />
                    <br />
                    <strong>EMS passif :</strong> charge depuis le surplus PV,
                    décharge pour couvrir la demande.
                  </div>
                  {profile.mode === 'autoconso' && (
                    <div className="profile-badge suggested">
                      ★ Recommandé pour ce profil
                    </div>
                  )}
              ...
              {profile.mode === 'mixed' && (
                <div className="info-box" style={{ marginTop: 16 }}>
                  ℹ️ Ce profil est mixte — les deux stratégies sont pertinentes.
                  Choisissez selon la priorité économique du client.
                </div>
              )}
            </div>
          )}
          ...
          {phase === 'computing' && (
            <div className="computing">
              ...
              <p>
                Stratégie :{' '}
                <strong>
                  {analysisMode === 'peakshaving'
                    ? 'Peak Shaving'
                    : 'Autoconsommation'}
                </strong>{' '}
                · {data?.length?.toLocaleString()} intervalles
              </p>
            </div>
          )}
                  ...
                  <>
                    ...
                    <div className="stats">
                      <div className="scard green">
                        ...
                        <div className="val">
                          {psOptimal?.batterySizeKWh?.toFixed(0) ?? '—'}
                          <span className="unit">kWh</span>
                        </div>
                        <div className="sub">
                          taux {psOptimal?.rate}% · ROI{' '}
                          {psOptimal?.roi?.toFixed(1)} ans
                        </div>
                      </div>
                      <div className="scard blue">
                        <div className="lbl">Économies annuelles optimales</div>
                        <div className="val">
                          {fmt(psOptimal?.annualSavings)}
                      ...
                      <div
                        className={`scard ${
                          psOptimal?.roi && psOptimal.roi < 10
                            ? 'green'
                            : 'amber'
                        }`}
                      >
                        ...
                        <div className="val">
                          {psOptimal?.roi?.toFixed(1) ?? '—'}
                          <span className="unit">ans</span>
                        </div>
                        <div className="sub">
                          {fmt(
                            psOptimal ? getPrice(psOptimal.batterySizeKWh) : 0
                          )}{' '}
                          CHF investissement
                        </div>
                      </div>
                    ...
                    {psOptimal &&
                      (() => {
                        const optSim = runSim(
                          data,
                          baseMaxKW * (1 - psOptimal.rate / 100),
                          psOptimal.batterySizeKWh,
                          'peakshaving',
                          hpHc
                        );
                        return (
                          <BeforeAfter
                            base={psCurve.base}
                            sim={optSim}
                            capKWh={psOptimal.batterySizeKWh}
                            prixPuissance={prixPuissance}
                            hpHc={hpHc}
                            prixAchat={prixAchat}
                            label="optimal"
                          />
                    ...
                    <div className="sec">
                      <div className="sec-title">
                        ...
                        />
                        ROI par taux de peak shaving
                      </div>
                      <div className="card">
                        <ResponsiveContainer width="100%" height={200}>
                          <LineChart
                            data={psRoiCurve.filter(
                              (d) => d.roi !== null && d.roi < 30
                            )}
                            margin={{ top: 5, right: 20, left: 10, bottom: 16 }}
                          >
                            <CartesianGrid
                              stroke="#e8eef5"
                              strokeDasharray="3 3"
                        ...
                        >
                          ★ = taux optimal ({psOptimal?.rate}%, ROI{' '}
                          {psOptimal?.roi?.toFixed(1)} ans) · Explorez ⚡
                          Dimensionnement pour ajuster
                        </div>
                      </div>
                    </div>
                  </>
                  ...
                  <>
                    <div className="stats">
                      <div className="scard green">
                        ...
                        <div className="val">
                          {psScenario?.batterySizeKWh?.toFixed(0) ?? '—'}
                          <span className="unit">kWh</span>
                        </div>
                        <div className="sub">
                      ...
                      <div className="scard teal">
                        ...
                        <div className="val">
                          {psScenario?.threshold?.toFixed(1) ?? '—'}
                          <span className="unit">kW</span>
                        </div>
                      ...
                      <div className="cfg-grid">
                        <div>
                          ...
                          <div className="slider-row">
                            <input
                              ...
                              value={sliderRate}
                              onChange={(e) =>
                                setSliderRate(Number(e.target.value))
                              }
                              onMouseUp={(e) =>
                          ...
                          <div className="slider-sub">
                            {sliderRate !== selectedRate ? (
                              <span style={{ color: 'var(--muted)' }}>
                                Relâchez pour calculer ce scénario…
                              </span>
                            ) : (
                              <>
                                Seuil :{' '}
                                <strong style={{ color: 'var(--text)' }}>
                                  {psScenario?.threshold?.toFixed(1)} kW
                                </strong>
                                &nbsp;·&nbsp; Batterie :{' '}
                                <strong style={{ color: 'var(--green)' }}>
                                  {psScenario?.batterySizeKWh?.toFixed(0)} kWh
                                </strong>
                                &nbsp;·&nbsp; Prix :{' '}
                                <strong style={{ color: 'var(--amber)' }}>
                                  {fmt(psScenario?.prix)} CHF
                                </strong>
                              </>
                            )}
                          </div>
                          {psOptimal && (
                            <button
                              ...
                              onClick={() => {
                                setSliderRate(psOptimal.rate);
                                setSelectedRate(psOptimal.rate);
                              }}
                            >
                        ...
                        >
                          ...
                          >
                            ...
                            Peak après :{' '}
                            <strong style={{ color: 'var(--green)' }}>
                              {psScenario?.maxGridKW?.toFixed(1)} kW
                            </strong>
                            <br />
                            Données :{' '}
                        ...
                        >
                          ...
                          >
                            ...
                            Écon./an :{' '}
                            <strong style={{ color: 'var(--green)' }}>
                              {fmt(psScenario?.annualSavings)} CHF
                            </strong>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="card">
                        <ResponsiveContainer width="100%" height={250}>
                          ...
                          >
                            ...
                            />
                            <ReferenceLine
                              y={psScenario?.threshold}
                              stroke="#d97706"
                              strokeDasharray="5 3"
                              strokeWidth={1.5}
                              label={{
                                value: `Seuil ${psScenario?.threshold?.toFixed(
                                  0
                                )} kW`,
                                position: 'insideTopRight',
                                fill: '#d97706',
                              ...
                              {points
                                ...
                                .map((p) => (
                                  ...
                                  >
                                    ...
                                    <td className="blu">
                                      {fmt(p.savPui ?? p.savingsPuissance)} CHF
                                    </td>
                                    <td>
                                      {fmt(p.savEn ?? p.savingsEnergy)} CHF
                                    </td>
                  ...
                  <>
                    {!hasPV && (
                      <div className="warn-box">
                        ⚠️ Aucune production PV dans ce fichier. L'analyse
                        d'autoconsommation n'est pas significative. Considérez
                        de passer en mode Peak Shaving.
                      </div>
                    )}
                    {acBase.selfConsRate > 0.88 && hasPV && (
                      <div className="info-box">
                        ℹ️ Taux d'autoconsommation sans batterie déjà élevé (
                        {baseNoSelfCons}%) — la batterie améliorera peu ce taux.
                        Le levier principal pour ce profil est le peak shaving.
                        Le calcul inclut néanmoins les économies de puissance
                        accessoires.
                      </div>
                    )}
                    ...
                    <div className="stats">
                      ...
                      <div className="scard blue">
                        ...
                        <div className="val">
                          {fmt(optimal.annualSavings)}
                          <span className="unit">CHF</span>
                        </div>
                        <div className="sub">
                          énergie + puissance incidentale
                        </div>
                      </div>
                      <div
                        className={`scard ${
                          acScenario?.roi && acScenario.roi < 10
                            ? 'green'
                            : 'amber'
                        }`}
                      >
                        ...
                        <div className="val">
                          {acScenario?.roi != null
                            ? `${acScenario.roi.toFixed(1)}`
                            : '—'}
                          <span className="unit">ans</span>
                        </div>
                        <div className="sub">
                          {fmt(getPrice(optimal.capKWh))} CHF investissement
                        </div>
                      </div>
                    ...
                    <div className="sec">
                      <div className="sec-title">
                        <span
                          ...
                          style={{ background: 'var(--blue)' }}
                        />
                        Économies & autoconsommation selon la taille de batterie
                      </div>
                      <div className="card">
                        <ResponsiveContainer width="100%" height={230}>
                          <ComposedChart
                                  ...
                                  >
                                    ...
                                    {payload.map((p, i) => (
                                      ...
                                      >
                                        ...
                                        />
                                        <span style={{ color: '#3d5166' }}>
                                          {p.name}:
                                        </span>
                                        <strong>
                                          {p.value?.toFixed(1)}{' '}
                                          {p.name.includes('%') ||
                                          p.name.includes('cons')
                                            ? '%'
                                            : 'CHF'}
                                        </strong>
                                      </div>
                  ...
                  <>
                    <div className="stats-3">
                      <div className="scard green">
                        ...
                        <div className="sub">
                          sans batterie : {baseNoSelfCons}%
                        </div>
                      </div>
                      <div className="scard blue">
                        <div className="lbl">Taux d'autarcie avec batterie</div>
                        <div className="val">
                          {((acScenario?.autarkyRate ?? 0) * 100).toFixed(1)}
                          <span className="unit">%</span>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="slider-row">
                            <input
                              type="range"
                              min={0}
                              max={acPoints[acPoints.length - 1]?.capKWh ?? 200}
                              step={
                                (acPoints[acPoints.length - 1]?.capKWh ?? 200) /
                                40
                              }
                            ...
                            <div className="slider-val">
                              {(selectedAcCap ?? optimal.capKWh).toFixed(0)} kWh
                            </div>
                          </div>
                          <div className="slider-sub">
                            Prix :{' '}
                            <strong style={{ color: 'var(--amber)' }}>
                              {fmt(getPrice(selectedAcCap ?? optimal.capKWh))}{' '}
                              CHF
                            </strong>
                            &nbsp;·&nbsp; Autoconso :{' '}
                            <strong style={{ color: 'var(--green)' }}>
                            ...
                            &nbsp;·&nbsp; Économies :{' '}
                            <strong style={{ color: 'var(--green)' }}>
                              {fmt(acScenario?.annualSavings)} CHF/an
                            </strong>
                          </div>
                          <button
                            className="btn-outline"
                          ...
                          >
                            ★ Revenir à la capacité optimale ({optimal.capKWh}{' '}
                            kWh)
                          </button>
                        </div>
                        <div
                    ...
                    <div className="two-col">
                      <div className="sec">
                        <div className="sec-title">
                          <span className="dot" />
                          Avant / Après batterie
                        </div>
                        <div className="card">
                          {[
                            {
                              label: 'Sans batterie',
                              sc: acBase.selfConsRate,
                              au: acBase.autarkyRate,
                              color: '#cbd5e1',
                              tc: '#7a90a8',
                            },
                            {
                              label: `Avec batterie (${(
                                selectedAcCap ?? optimal.capKWh
                              ).toFixed(0)} kWh)`,
                              sc: acScenario?.selfConsRate ?? 0,
                              au: acScenario?.autarkyRate ?? 0,
                              color: '#0d9f6e',
                              tc: '#0d9f6e',
                            },
                          ].map((col) => (
                            <div key={col.label} style={{ marginBottom: 20 }}>
                              ...
                              {[
                                {
                                  lbl: 'Autoconsommation',
                                  val: col.sc,
                                  c: col.color,
                                },
                                { lbl: 'Autarcie', val: col.au, c: '#1a6fbf' },
                              ].map((g) => (
                      ...
                      <div className="sec">
                        ...
                        <div className="card">
                          {[
                            {
                              label: 'Production PV',
                              val: acScenario?.pvKWh,
                              color: '#f59e0b',
                            },
                            {
                              label: 'Consommation',
                              val: acScenario?.consoKWh,
                              color: '#1a6fbf',
                            },
                            {
                              label: 'Achat réseau',
                              val: acScenario?.gridKWh,
                              color: '#dc2626',
                            },
                            {
                              label: 'Injection réseau',
                              val: acScenario?.injKWh,
                              color: '#0d9f6e',
                            },
                            {
                              label: 'PV autoconsommé',
                              val: acScenario?.selfConsKWh,
                              color: '#0891b2',
                            },
                          ].map((row) => {
                            const max = Math.max(
                              acScenario?.pvKWh || 1,
                              acScenario?.consoKWh || 1
                            );
                            return (
